...
function gui_gamma_correction_OpeningFcn(hObject, eventdata, handles, varargin)
    ...
    rgb_small = evc_gamma_correction(rgb_small, gamma, get(handles.checkbox1, 'Value'));
    m = max(rgb_small(:));
...
function varargout = gui_gamma_correction_OutputFcn(hObject, eventdata, handles) 
    ...
    varargout{1} = evc_gamma_correction(handles.input, gamma, get(handles.checkbox1, 'Value'));
...
function updateGamma(hObject, handles)
    ...
    axes(handles.ax_picture);
    cla;
    RGB_small = evc_gamma_correction(handles.RGB_CLIP_small, gamma, get(handles.checkbox1, 'Value'));
    m = max(RGB_small(:));
    imshow(RGB_small ./ m);
    axes(handles.ax_hist);
    cla;
